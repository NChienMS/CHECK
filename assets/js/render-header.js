...
// render-header.js - Render menu + xá»­ lÃ½ dropdown + mobile menu
...
function createMenuItem(item) {
  ...
  if (item.children && item.children.length) {
    ...
    // NÃºt má»Ÿ submenu cho mobile
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "submenu-toggle";
    btn.setAttribute("aria-label", "Má»Ÿ danh má»¥c con");
    btn.innerHTML = "â–¾";
    li.appendChild(btn);
...
function renderMobileMenuLogo() {
  ...
  brandText.textContent = "ÄÃ Má»¸ NGHá»† QUANG LUYáº¾N";
...
window.renderHeaderMenu = function (menuData) {
  ...
  // CÅ©ng render menu cho mobile drawer
  const mobileNav = document.getElementById("mobile-nav");
  if (mobileNav) {
    mobileNav.innerHTML = "";
...
window.initHeaderUI = function (siteConfig) {
  ...
  if (headerSocial && siteConfig) {
    ...
    if (siteConfig.social?.tiktokUrl) socials.push({href: siteConfig.social.tiktokUrl, title:'TikTok', icon:`<img src="${withBase("/assets/img/general/tiktok.png")}" alt="TikTok">`});
    socials.forEach(s=>{
      const a=document.createElement('a');
